  ...
  routes: [
    ...
    // =============================================================================
      ...
      children: [
        ...
        // =============================================================================
        {
          ...
          //redirect: '/admin/default' IS NOW HANDLED BY THE GUARD ON THE BOTTOM OF THE PAGE
        },
        ...
        //admin pages
        {
          path: 'admin',
          name: 'adminpage',
          component: () => import('./views/-admin/DefaultPage.vue'),
          meta: {
            rule: 'admin'
        ...
        //company pages
        {
          path: 'company/',
          name: 'companypage',
          component: () => import('./views/-company/DefaultPage.vue'),
          meta: {
            breadcrumb: [
              { title: 'Home', url: '/' },
        ...
        },
        {
          ...
          meta: {
            breadcrumb: [
              ...
              { title: 'Company Settings', active: true }
            ],
            pageTitle: 'Settings',
            rule: 'editor'
          }
        ...
        //generic pages
        {
          path: '/addcredits',
          name: 'addcredits',
          component: () => import('./views/-pages/AddCredits.vue'),
          meta: {
            breadcrumb: [
              { title: 'Home', url: '/' },
              { title: 'Credits', active: true }
            ],
...

        {
          path: '/websocket/test',
          name: 'websockettest',
          component: () => import('./views/-websockets/Test.vue'),
          meta: {
            rule: 'editor'
    ...
    // =============================================================================
    {
      ...
      children: [
        ...
        {
          path:  '/registry-confirmed',
          name: 'registry-confirmed',
          component: () => import('@/views/-pages/register/Confirmed.vue'),
          meta: {
            rule: 'anyone'
          }
        },
...
router.beforeEach((to, from, next) => {
  const userRole = store.state.AppActiveUser.userRole
  //if (userRole) acl.change(userRole)
  // console.log(`userRole: ${  userRole}`)
  //console.log(`acl route get: ${  acl.get[0]}`)

  console.log(`ok we came from ${  from.path  } and we are going to ${  to.path } where rule is: ${  to.meta.rule}`)
  //console.log(from.path)
  //First, we will push users who are not logged in to login prompt for all pages except those viewable by anyone
  ...
  //Then we will check if request is at root, then we act accordingly (redirect from login is handled  there)
  if (to.path === '/') {
    console.log(userRole)
    let nextpage = '' //just to set something..
    if (userRole === 'admin' || userRole === 'superadmin') nextpage = '/admin'
    else if (userRole === 'editor' || userRole === 'dealer') nextpage = '/company'
    else nextpage = '/user' //obviously there will be more of these

    if (from.path !== nextpage) router.replace({ path: nextpage}) //if we route to where we came from Vue.router crashes..
    else {
      console.log('error!!')
      return 
    }
